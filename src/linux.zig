...
const ALLCRED_SCHEMA = c.secret_schema_new(
    "org.allcred.Credential",
    c.SECRET_SCHEMA_NONE,
    "service\x00",
    c.SECRET_SCHEMA_ATTRIBUTE_STRING, // 属性 1
    "account\x00",
    c.SECRET_SCHEMA_ATTRIBUTE_STRING, // 属性 2
    @as(?*anyopaque, null), // 必须显式转成 C 识别的 null 指针作为结束标记
);
...
pub fn set(cred: Credential, diagnostic: *CredentialDiagnostic) !void {
    if (diagnostic) |diag| {
        diag.* = CredentialDiagnostic{ .service = cred.service, .account = cred.account };
    }
    ...
    const password_z = try std.fmt.allocPrintSentinel(allocator, "{s}", .{cred.secret}, '\x00');
    ...
    if (is_success != 0) {
        ...
        if (set_error) |err| {
            ...
            std.debug.log("Failed to set credential: {s}", .{err.message});
            if (diagnostic) |diag| {
                diag.*.error_message = err.message;
            }
            return CredentialError.SetFailed;
        } else {
            std.debug.log("Failed to set credential: Unknown error", .{});
            if (diagnostic) |diag| {
...
pub fn get(allocator: std.mem.Allocator, service: []const u8, account: []const u8, diagnostic: ?*CredentialDiagnostic) !Credential {
    ...
    // 前置工作
    if (diagnostic) |diag| {
        diag.* = CredentialDiagnostic{ .service = service, .account = account };
    }

    var get_error: ?*c.GError = null;

    const service_z = try std.fmt.allocPrintSentinel(allocator, "{s}", .{service}, '\x00');
    const account_z = try std.fmt.allocPrintSentinel(allocator, "{s}", .{account}, '\x00');
    ...
    if (password_ptr) |value| {
        const password = std.mem.trimRightZ(value);
        // 这里直接返回一个 Credential 结构体，包含 service、account 和 password 字段
        var credential_temp = try allocator.alloc(Credential, 1);

        credential_temp[0] = Credential{
        ...
        return credential_temp[0];
    } else {
        if (get_error) |err| {
            defer c.g_error_free(get_error);

            std.debug.print("Failed to get credential: {s}", .{err.message});
            if (diagnostic) |diag| {
                diag.*.error_message = err.message;
            }
...
pub fn delete(allocator: std.mem.Allocator, service: []const u8, account: []const u8, diagnostic: ?*CredentialDiagnostic) !void {
    ...
    if (is_success != 0) {
        ...
        if (delete_error) |err| {
            ...
            std.debug.print("Failed to delete credential: {s}", .{err.message});
            if (diagnostic) |diag| {
                diag.*.error_message = err.message;
            }
